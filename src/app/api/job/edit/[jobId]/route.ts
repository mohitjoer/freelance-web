...
export async function PATCH(req: NextRequest, context: { params: { jobId: string } }) {
  try {
    // Fix: Await params before accessing properties
    const { jobId } = await context.params;
    ...
    if (['in-progress', 'cancelled', 'completed'].includes(job.status)) {
      return NextResponse.json({ success: false, message: `${job.status} jobs cannot be edited.` }, { status: 400 });
    }
