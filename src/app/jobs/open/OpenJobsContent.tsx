...
function OpenJobsContent({ initialJobs, message }: OpenJobsContentProps) {
  ...
  const filteredJobs = useMemo(() => {
    ...
    return initialJobs.filter(job => {
      ...
      return matchesSearch && (!category || job.category.toLowerCase() === category);
    });
  }, [searchQuery, categoryFilter, initialJobs]);
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, idx) => (
              <div key={`item-${idx}`} className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {filteredJobs.map(job => (
              <div
                key={job._id}
                className="bg-white border border-gray-200 rounded-xl p-6 hover:shadow-lg transition duration-200 hover:border-blue-300 group"
              >
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between text-sm">
                    ...
                    <div className="flex items-center space-x-1 text-gray-500">
                      ...
                      <span>{dateFormatter.format(new Date(job.deadline))}</span>
                    </div>
                  </div>

                  <button
                    onClick={() => router.push(`/jobs/${job.jobId}`)}
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-lg text-sm font-semibold transition duration-200 shadow-sm hover:shadow-md"
                  >
                    View Details & Apply
                  </button>
                </div>
